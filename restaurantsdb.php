...
#Function 1:
function getRestaurants($query = null, $order = "1"){      // creamos una funcion con una array que contiene 5 arrays
  ...
  global $db;
    ...
    $sql .= " ORDER BY r.name $ord";




/*
if (isset($_REQUEST["search"]== $query)) {
  echo "HOLA";
}
*/
 ...
 /* $sql = "SELECT r.id,r.name,r.description,r.openingHours,r.priceCategory,r.locality,r.route,
  ...
  if (!$data){
    echo "No restaurants with this name";
  } 
...
function getRestaurant($id) {
  ...
  $sql = "SELECT r.id,r.name,r.description,r.openingHours,r.priceCategory,r.locality,r.route,
  r.streetNumber,r.postalCode,r.latitude,r.longitude,r.phoneNumber,r.website,r.email,
  r.rating,r.isTrending,Image.filePath FROM Restaurant r,Image WHERE r.id=Image.restaurantId AND r.id=$id";
  $result = mysqli_query ($db, $sql);
  #$data = mysqli_fetch_all($result, MYSQLI_ASSOC);
